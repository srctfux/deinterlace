...
BOOL GreedyClipping(DEINTERLACE_INFO *info)
{
	int Line;
	int	LoopCtr;
...
{
	if(Setting > -1 && Setting < DI_GREEDY_SETTING_LASTONE)
	{
		return &(DI_GreedySettings[Setting]);
	}
	else
	{
		return NULL;
...
{
	int i;
	for(i = 0; i < DI_GREEDY_SETTING_LASTONE; i++)
	{
		Setting_ReadFromIni(&(DI_GreedySettings[i]));
	}
}
...
void DI_Greedy_WriteSettingsToIni()
{
	...
	for(i = 0; i < DI_GREEDY_SETTING_LASTONE; i++)
	{
		Setting_WriteToIni(&(DI_GreedySettings[i]));
	}
}
