...
class PeriodBouncer {
	public:
		...
		double position() {
			...
			if (val > m_amplitude) val = fabs(2*m_amplitude-val); // Adjust - 50% into the phase, turn around and go down
			return val + m_offset;
		}
...
class AspectFilter {
	public:
		AspectFilter() { firstChild = next = NULL; bEnabled = TRUE; }
		virtual ~AspectFilter() { delete firstChild; delete next; }
...
class OverscanAspectFilter : public AspectFilter {
	public:
		...
		virtual LPCSTR getFilterName() { return "OverscanAspectFilter"; }
		virtual void DebugDump(FILE *f) { fprintf(f,"Overscan = %i\n",overscan); }
...
class OrbitAspectFilter : public AspectFilter {
	public:
		...
		~OrbitAspectFilter() { delete xOrbit; delete yOrbit; }
		...
		virtual LPCSTR getFilterName() { return "OrbitAspectFilter"; }
		virtual void DebugDump(FILE *f) { fprintf(f,"xOrbit = %lf, yOrbit = %lf\n",xOrbit->position(), yOrbit->position()); }
...
class BounceDestinationAspectFilter : public AspectFilter {
	public:
		...
		~BounceDestinationAspectFilter() { delete bouncer; }
		...
		virtual LPCSTR getFilterName() { return "BounceDestinationAspectFilter"; }
		virtual void DebugDump(FILE *f) { fprintf(f,"position = %lf\n",bouncer->position()); }
...
class PositionDestinationAspectFilter : public AspectFilter {
	public:
		...
		virtual LPCSTR getFilterName() { return "PositionDestinationAspectFilter"; }
		virtual void DebugDump(FILE *f) { fprintf(f,"xPos = %lf, yPos = %lf\n",xPos,yPos); }
...
class CropAspectFilter : public AspectFilter {
	public:
		...
		virtual LPCSTR getFilterName() { return "CropAspectFilter"; }
...
class UncropAspectFilter : public AspectFilter {
	public:
		...
		virtual LPCSTR getFilterName() { return "UncropAspectFilter"; }
};
...
class PanAndZoomAspectFilter : public AspectFilter {
	public:
		PanAndZoomAspectFilter(double _xPos, double _yPos, double _xZoom, double _yZoom) {
			xPos = _xPos; yPos = _yPos; xZoom = _xZoom; yZoom = _yZoom;
		}
		...
		virtual LPCSTR getFilterName() { return "PanAndZoomAspectFilter"; }
		virtual void DebugDump(FILE *f) { fprintf(f,"xPos = %lf, yPos = %lf, xZoom = %lf, yZoom = %lf\n",xPos,yPos,xZoom,yZoom); }

	protected:
			double xPos, yPos, xZoom, yZoom;
};
...
class ScreenSanityAspectFilter : public AspectFilter {
	public:
		virtual BOOL adjustAspect(AspectRectangles &ar) {
			...
			if (SrcSizeAlign > 1) ar.rCurrentOverlaySrc.align(SrcSizeAlign);
			if (DestSizeAlign > 1) ar.rCurrentOverlayDest.align(DestSizeAlign);
		...
		virtual LPCSTR getFilterName() { return "ScreenSanityAspectFilter"; }
};
...
class ResizeWindowAspectFilter : public AspectFilter {
	public:
		virtual BOOL adjustAspect(AspectRectangles &ar) {
			if (!bIsFullScreen) {
				...
				if (!currentClientRect.tolerantEquals(newRect,8)) { // Do we match????
					...
					#ifdef __ASPECTFILTERDEBUG__
						fprintf(debugLog, "New Window Pos     :"); currentClientRect.DebugDump(debugLog);
					#endif
		...
		virtual LPCSTR getFilterName() { return "ResizeWindowAspectFilter"; }
};
